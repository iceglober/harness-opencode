...
- **OpenCode plugins**
  - Local: `notify` (OS notifications for question tool), `autopilot` (opt-in completion-tag loop driver with skeptical verifier gate — only activates on explicit `/autopilot` invocation or `/plan-loop` handoff, never on normal orchestrator sessions), `auto-update` (opportunistic self-update at session start — see [docs/installation.md#auto-update](docs/installation.md#auto-update)), `cost-tracker` (running LLM spend by provider/model at `~/.glorious/opencode/costs.json`; view with `/costs`; disable with `GLORIOUS_COST_TRACKER=0`, override path with `GLORIOUS_COST_TRACKER_DIR`)
