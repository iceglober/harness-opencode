...
- The autopilot subsystem (bundled in `@glorious/harness-opencode`) will inject continuation messages if your session goes idle mid-plan, drive the completion-promise → verifier loop, and cap iterations. Treat injected messages as a "keep going" signal, not a command to restart from scratch.
